	...
	private: /** delimitador de alcance privado */
		...
		class Nodo{ /** definición de la clase Nodo */
			public: /** todos sus atributos y métodos son públicos */
				...
				Nodo* insertar(int key, Nodo* nodo1, Nodo* nodo2, ArbolRN& arbol){ /** recibe un key de tipo entero, los punteros de 2 nodos y la refrencia del árbol al que pertenece */
					...
					if(this->key != key){ /** se agrega si el key recibido como parámetro no se ha agregado previamente */
						...
						if(nodo1->hijo[lado]){ /** si existe un nodo intermedio (nodo llave) de dicho lado */

							if(nodo1->hijo[IZQ]->color == 'R' && nodo1->hijo[DER]->color == 'R'){ /** verifica si ambos hijos de nodo son rojos */
								arbol.colorFlip(nodo1); /** en tal caso, ejecuta el color-flip, donde el padre se vuelve rojo y los hijos negros */
							}
							...
							arbol.iterador.setPila(arbol.pila); /** se indica al iterador el buffer con el que trabajará */
						...
						else{ /** si llega a un nodo hoja (los nodos hoja no tienen hijos) */
				...
				/** método que nos dice si un dato existe o no dentro del árbol */
				int existe(int dato){ /** recibe el dato de tipo T como parámetro */
				...
				/** método que imprime en orden, la llave y dato de los nodos respectivos (hijo izquierdo si existe, padre(este nodo) y el hijo derecho si existe) */
				ostream& imprimir(ostream& salida){ /** recibe y devuelve la referencia de un objeto de salida de flujo */
					if(hijo[IZQ]){ /** si el hijo izquierdo existe */
						hijo[IZQ]->imprimir(salida); /** llama de forma recursiva al metodo imprimir de el hijo izquierdo*/
					}
    			salida << "Dato: "<< dato << " Key:" << key << "       "; /**manda a salida el dato y deja un espacio*/
					if(hijo[DER]){ /** si el hijo derecho existe */
						hijo[DER]->imprimir(salida); /** llama de forma recursiva el metodo imprimir del hijo derecho */
					}
					return salida; /** retorna el objeto de salida de flujo */
				}
		};

		/** El puntero a Nodo "raiz" y la pila son atributos (privados) de la clase ArbolRN */
		Nodo* raiz; /** puntero a Nodo que almacena la raíz del árbol rojinegro */
		/** utilizará un vector de punteros a Nodo como si fuera un buffer, para así guardar el camino que se traza al bajar por el árbol
		y así utilizar el iterador para realizar métodos como las rotaciones */
		Pila pila;

	public: /** delimitador de acceso público */

		/** la clase Iterador es clase interna de ArbolRN */
		class Iterador{
			public:
				Pila pila; /** instancia de pila dentro de la clase iterador */
				/** tiene cuatro punteros para agilizar las rotaciones para equilibrar el árbol roji-negro */
			...
			/** este método es invocado por el constructor y el destructor para inicializar los punteros a Nodo en nulo, no recibe parámetros */
			void _initPtr(){
				bisAbuelo = 0;
				abuelo = 0;
				padre = 0;
			...
			/** constructor por omisión de la clase iterador, utiliza el método inicializador de punteros para establecerlos en null */
			Iterador(){
				_initPtr();
			}

			// hasta aquí llegué 2
			/** destructor de la clase Iterador el cual no tiene que matar nada porque los nodos los mata el arbol pero es efectivo hacer que cuando se muera sus punteros queden en nulo */
			~Iterador(){
				_initPtr();
			}
			...
			void operator--(){
				actual = padre; // actual se devuelve una posicion que corresponde al padre
				padre = abuelo; // padre se devuelve una posicion que corresponde al abuelo
				abuelo = bisAbuelo; // abuelo se devuelve una posicion que corresponde al bisAbuelo
...
// cuando compruebe si existe un nodo, que verifique además del dato, la llave (key)
...
// linea 429 le agregue un setPila a iterador de arbol
// linea 60 corregimos un error de inicializacion en el constructor por omision de Pila
